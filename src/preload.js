...
function ensureUpdateHud() {
  let hud = document.getElementById('tj-desktop-update-hud');
  if (hud) return hud;

  const style = document.createElement('style');
  style.id = 'tj-desktop-update-hud-style';
  style.textContent = `
    #tj-desktop-update-hud {
      position: fixed;
      right: 18px;
      bottom: 18px;
      ...
      border: 1px solid rgba(116, 173, 255, .28);
      border-radius: 14px;
    ...
    #tj-desktop-update-hud[hidden] { display: none !important; }
    #tj-desktop-update-hud .tj-up-row { display:flex; align-items:center; gap:9px; min-width:0; }
    #tj-desktop-update-hud .tj-up-dot { width:9px; height:9px; border-radius:999px; flex:0 0 auto; background:#6e8cff; box-shadow:0 0 16px rgba(110,140,255,.65); }
    #tj-desktop-update-hud[data-state="downloading"] .tj-up-dot,
    #tj-desktop-update-hud[data-state="installing"] .tj-up-dot { animation:tjUpdatePulse 1.15s ease-in-out infinite; }
    #tj-desktop-update-hud[data-state="ready"] .tj-up-dot { background:#52e0af; box-shadow:0 0 16px rgba(82,224,175,.55); }
    #tj-desktop-update-hud[data-state="error"] .tj-up-dot { background:#ff6f86; box-shadow:0 0 16px rgba(255,111,134,.55); }
    #tj-desktop-update-hud .tj-up-main { min-width:0; flex:1; }
    #tj-desktop-update-hud .tj-up-title { font-size:12px; line-height:1.2; font-weight:800; letter-spacing:.02em; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
    #tj-desktop-update-hud .tj-up-detail { margin-top:3px; font-size:10.5px; line-height:1.3; color:#9fb0cf; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
    #tj-desktop-update-hud .tj-up-pct { font-size:12px; font-weight:900; color:#cfe0ff; min-width:38px; text-align:right; }
    #tj-desktop-update-hud .tj-up-track { position:relative; height:5px; margin-top:9px; border-radius:99px; overflow:hidden; background:rgba(130,155,205,.16); }
    #tj-desktop-update-hud .tj-up-fill { position:absolute; inset:0 auto 0 0; width:0%; border-radius:inherit; background:linear-gradient(90deg,#5cc8ff,#7d7cff,#51e2c2); transition:width .2s ease; }
    #tj-desktop-update-hud[data-indeterminate="true"] .tj-up-fill { width:35% !important; animation:tjUpdateSlide 1.05s ease-in-out infinite; }
    #tj-desktop-update-hud .tj-up-actions { display:flex; justify-content:flex-end; gap:7px; margin-top:9px; }
    #tj-desktop-update-hud button { appearance:none; border:1px solid rgba(130,168,235,.25); border-radius:9px; background:rgba(100,137,210,.12); color:#dce8ff; padding:6px 9px; font:700 10px/1.1 Inter,ui-sans-serif,system-ui,sans-serif; cursor:pointer; }
    #tj-desktop-update-hud button:hover { background:rgba(100,137,210,.22); }
    #tj-desktop-update-hud button.tj-up-primary { border-color:rgba(101,218,192,.38); background:rgba(48,183,155,.16); color:#9ff2dc; }
