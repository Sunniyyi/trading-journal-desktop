...
function postToPage(window, message) {
  window.postMessage(message, '*');
}

function installPageBridge({ window, document, ipcRenderer }) {
  window.addEventListener('DOMContentLoaded', () => {
    postToPage(window, { source: 'fxr-extension', type: 'FXR_EXTENSION_BRIDGE_READY', at: Date.now(), desktop: true });
  ...
  ipcRenderer.on('fxr:status', (_event, status) => {
    postToPage(window, { source: 'fxr-extension', type: 'FXR_EXTENSION_STATUS', ...(status || {}), desktop: true });
  });

  ipcRenderer.on('fxr:import', (_event, item) => {
    postToPage(window, { source: 'fxr-extension', type: 'FXR_IMPORT_TRADE', item, desktop: true });
  });
...
module.exports = { installPageBridge };
